...
f = linspace(fmin, fmax, fsteps); // Initialize array for frequency
rho_p = zeros(1, fsteps); // Initialize array for reflection coefficient
...
for i = 1:fsteps
  lambda = c/f(i);
  k = 2*%pi/lambda;
  z_in = antenna_zin(l, k, a);
  rho_p(i) = z_rho(z_in, z0);
end

// Plot reflection coefficient vs frequency
figure(2);
scf();
plot(f/1e9,rho_t*ones(1,length(f)),'k--',f/1e9, rho_p, 'b');
xlabel(sprintf('Frequency (GHz) for l = %.3f', l));
ylabel('Reflection Coefficient (|rho|)');
xgrid(1);
